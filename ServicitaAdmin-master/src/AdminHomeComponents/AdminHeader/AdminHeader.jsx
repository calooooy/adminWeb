    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                ...
                const response = await Axios.get(`http://3.26.59.191:/notifications/getNotifications/${userId}`);
                const notificationsData = response.data.data;

                const notificationInfoData = [];

                for (const notification of notificationsData) {
                    const otherUserResponse = await Axios.get(`http://3.26.59.191:/admin/getUser/${notification.otherUserId}`);
