...
function ResolvedComplaints() {
	...
	useEffect(() => {
		...
		const fetchReports = async () => {
			try {
				...
				const response = await Axios.get('https://192.168.1.4:5001/report/getReports');
				...
				for (const report of reportsData) {
					...
					const reporterResponse = await Axios.get(`https://192.168.1.4:5001/admin/getUser/${report.reporterId}`); //for reporter profileImage
					const reportedResponse = await Axios.get(`https://192.168.1.4:5001/admin/getUser/${report.reportedId}`); //for reported role
	...
	const handleIgnore = async (record) => {
		...
		try{
			await Axios.delete(`https://192.168.1.4:5001/report/deleteReport/${record.id}`)
			setChanges(true)
			setLoading(true)
		} catch (error) {
	...
	const renderActions = (record) => {
	  ...
	  const handleSubMenuClick = (record, action) => {
		...
		try {
		  ...
		  Axios.patch('https://192.168.1.4:5001/admin/suspendUser', userData)
			.then((response) => {
		  ...
		  <div className='message-container'>
			{showSpinner ? (
			  ...
			  filterByStatus.length === 0 ? (
				<div style={{padding: '10px 0px 0px 10px'}} >No current Resolved Complaints to review</div>
			  ) : (
