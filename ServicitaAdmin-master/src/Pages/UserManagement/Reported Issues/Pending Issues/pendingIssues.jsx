...
function PendingIssues() {
	...
	useEffect(() => {
		...
		const fetchReports = async () => {
			try {
				...
				const response = await Axios.get('http://3.26.59.191://report/getReports');
				...
				for (const report of reportsData) {
					...
					const reporterResponse = await Axios.get(`http://3.26.59.191://admin/getUser/${report.reporterId}`); //for reporter profileImage
					// const reportedResponse = await Axios.get(`http://3.26.59.191://admin/getUser/${report.reportedId}`); //for reported role
	...
	const handleIgnore = async (record) => {
		...
		try{
			await Axios.delete(`http://3.26.59.191://report/deleteReport/${record.id}`)
			setChanges(true)
			setLoading(true)
		} catch (error) {
	...
	const handleTakeAction = async (record) => {
		...
		try{
            await Axios.put(`http://3.26.59.191://report/updateReport/${record.id}`, {
                status: 'IN PROGRESS'
              });
			setChanges(true)
			setLoading(true)
	...
	return (
		<div className='reviewComplaints1'>
		  <div>
			<h1 className='DashboardHeader'>Pending Issues</h1>
		  </div>
		  <div className='message-container'>
			{showSpinner ? (
			  ...
			  filterByStatus.length === 0 ? (
				<div style={{padding: '10px 0px 0px 10px'}} >No current Pending Issues to review</div>
			  ) : (
				  ...
				  {changes ? (
					...
					<>
					  <div className='complaintsCount1' style={{padding: '10px 0px 0px 10px'}}>
						Pending Issues to Review: {filterByStatus.length}
					  </div>
